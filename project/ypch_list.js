...
function getTotalPaginations() {
    return casper.evaluate(function () {
        var p = jQuery("[class='pagination']");
        if (p.size() === 0) {
            return 1;
        }
        return p.find("span.page.gap").next().text().trim();
    });
}
...
function extractPage(cn, sp) {
    return casper.evaluate(function (c, s) {
        var r = "";
        jQuery(".row.local-listing").each(function () {
            ...
            if (!sa.charAt(iocbpc + 3).match(/\d/).length) {
                ...
                sa = sa.substring(0, sa.lastIndexOf(",")) + "\t" + sa.substring(sa.lastIndexOf(",") + 1, sa.length);
            }
...
if (okToGo) {
    ...
    var baseUrl = "http://yellow.local.ch/en/q";
